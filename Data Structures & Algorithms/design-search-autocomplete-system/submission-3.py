...
class AutocompleteSystem:
    ...
    def input(self, c: str) -> list[str]:
        # if finishing
        if c == "#":
            self.current_pointer.hot_degree += 1
            self.current_pointer = self.trie.root
            self.current_sentence = []
            return []
        else:
            ...
            if c not in self.current_pointer.children:
                self.current_pointer.children[c] = TrieNode()
            
            self.current_pointer = self.current_pointer.children[c]
            
            return self.trie.retrieve(self.current_pointer, self.current_sentence)